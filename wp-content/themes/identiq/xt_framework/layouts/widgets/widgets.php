...
class XTTwitterFeeds extends WP_Widget {
	...
	function XTTwitterFeeds() {
		$widget_ops = array( 'classname' => 'twitter-feeds', 'description' => __('A widget that displays a Twitter account feeds', 'identiq') );
		
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'twitter-feed-widget' );
		
		$this->WP_Widget( 'twitter-feed-widget', __('XT - Twitter Feeds', 'identiq'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Recent Tweets', 'account' => __('', 'identiq'), 'num' => 2 );
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title:', 'identiq'); ?></label>
			<input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" style="width:100%;" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'account' ); ?>"><?php _e('Twitter Account:', 'identiq'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'num' ); ?>"><?php _e('Tweets Number:', 'identiq'); ?></label>
...
class XTOrganizerTabs extends WP_Widget {
	...
	function XTOrganizerTabs() {
		$widget_ops = array( 'classname' => 'organizer-tabs', 'description' => __('Display a widget with Categories - Tags - Archives', 'identiq') );
		
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'organizer-tabs-widget' );
		
		$this->WP_Widget( 'organizer-tabs-widget', __('XT - Categories, Tags, Archives', 'identiq'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<li class="cats-arch-tags-widget">
			[xt_tabs_framed]
				...
				[xt_tab title="<?php _e("Categories", 'identiq'); ?>" icon="th-list"]
					<ul>
                       	<?php
				...
				[xt_tab title="<?php _e("Tags", 'identiq'); ?>" icon="tags"]
					<ul>
                        <?php
						$tags =  get_tags(); 
						$count = count($tags);
				...
				[xt_tab title="<?php _e("Archives", 'identiq'); ?>" icon="bookmark"]
					<ul>
                        <?php
...
class XTLatestProjects extends WP_Widget {
	...
	function XTLatestProjects() {
		$widget_ops = array( 'classname' => 'latest-projects', 'description' => __('A widget that displays a latest projects grid', 'identiq') );
		
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'latest-projects-widget' );
		
		$this->WP_Widget( 'latest-projects-widget', __('XT - Latest Projects', 'identiq'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'columns' ); ?>"><?php _e('Columns:', 'identiq'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'num' ); ?>"><?php _e('Number of Projects:', 'identiq'); ?></label>
			<input id="<?php echo $this->get_field_id( 'num' ); ?>" name="<?php echo $this->get_field_name( 'num' ); ?>" value="<?php echo $instance['num']; ?>" style="width:100%;" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'offset' ); ?>"><?php _e('(Optional) Offset:', 'identiq'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'categories' ); ?>"><?php _e('(Optional) Filters:', 'identiq'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'portfolio' ); ?>"><?php _e('(Optional) Portfolio:', 'identiq'); ?></label>
...
class XTPostTabs extends WP_Widget {
	...
	function XTPostTabs() {
		$widget_ops = array( 'classname' => 'post-tabs', 'description' => __('Display a widget with Recent - Popular - Comments Feeds', 'identiq') );
		
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'post-tabs-widget' );
		
		$this->WP_Widget( 'post-tabs-widget', __('XT - Recent, Popular, Comments', 'identiq'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<li class="recent-popular-comments">
			[xt_tabs_framed]
				...
				[xt_tab title="<?php _e("Recent", 'identiq'); ?>" icon="magic"]
					<ul>
									<?php
										...
										while ($r->have_posts()) :
											...
											?>
													<li<?php if($currentPost == $count)  echo ' class="last-child"'; ?>>
														...
														<div class="rpc_post_comments"><a href="<?php comments_link(); ?>"><?php comments_number( 'No Comments', '1 Comment', __('% Comments', 'identiq') ); ?></a></div>
													</li>
											<?php
				...
				[xt_tab title="<?php _e("Popular", 'identiq'); ?>" icon="star"]
					 <ul>
                                    <?php
				...
				[xt_tab title="<?php _e("Comments", 'identiq'); ?>" icon="microphone"]
					<ul>
                                    <?php
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'count-recent' ); ?>"><?php _e('Recent Posts Number:', 'identiq'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'count-popular' ); ?>"><?php _e('Recent Popular Number:', 'identiq'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'count-comments' ); ?>"><?php _e('Comments Number:', 'identiq'); ?></label>
...
class XTFacebookBox extends WP_Widget {
	...
	function XTFacebookBox() {
		$widget_ops = array( 'classname' => 'facebookbox-feeds', 'description' => __('A widget that displays a Facebook Like Box', 'identiq') );
		
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'facebook-box-widget' );
		
		$this->WP_Widget( 'facebook-box-widget', __('XT - Facebook Box', 'identiq'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		//Set up some default widget settings.
		$defaults = array( 'title' => 'Like Box', 'account' => __('', 'identiq'), 'num' => 2 );
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'account' ); ?>"><?php _e('Facebook Page Address:', 'identiq'); ?></label>
			<input id="<?php echo $this->get_field_id( 'account' ); ?>" name="<?php echo $this->get_field_name( 'account' ); ?>" value="<?php echo $instance['account']; ?>" style="width:100%;" />
		</p>
...
class XTFlickr extends WP_Widget {
	...
	function XTFlickr() {
		$widget_ops = array( 'classname' => 'flickr-feeds', 'description' => __('A widget that displays a Flickr account feeds', 'identiq') );
		
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'flickr-feed-widget' );
		
		$this->WP_Widget( 'flickr-feed-widget', __('XT - Flickr Feeds', 'identiq'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		//Set up some default widget settings.
		$defaults = array( 'title' => 'Flickr Feed', 'account' => __('', 'identiq'), 'num' => 2 );
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'account' ); ?>"><?php _e('Flickr Account:', 'identiq'); ?></label>
			<input id="<?php echo $this->get_field_id( 'account' ); ?>" name="<?php echo $this->get_field_name( 'account' ); ?>" value="<?php echo $instance['account']; ?>" style="width:100%;" />
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'num' ); ?>"><?php _e('Photo Number:', 'identiq'); ?></label>
			<input id="<?php echo $this->get_field_id( 'num' ); ?>" name="<?php echo $this->get_field_name( 'num' ); ?>" value="<?php echo $instance['num']; ?>" style="width:100%;" />
		</p>
