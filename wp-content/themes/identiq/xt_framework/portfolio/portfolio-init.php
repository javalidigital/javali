...
********************/
	...
	add_action( 'admin_enqueue_scripts', 'xt_portfolio_admin_style' );
...
function xt_portfolio_image_sizes() {
	if ( function_exists( 'add_image_size' ) ) { 
		...
		add_image_size( 'xt-portfolio', 600, 9999 );
...
/**************************/
	...
	{  
		...
		// CUSTOM EXTRA FIELDS
		$_client = (isset($custom["proj-client"])) ? $custom["proj-client"][0] : '';
		$_date = (isset($custom["proj-date"])) ? $custom["proj-date"][0] : '';
	?>  
		...
		<div class="xt-metabox">
			...
			<div class="single-project-options xt-input" style="background: transparent;">
				<label>Client</label>
				<input type="text" name="proj-client" value="<?php echo $_client; ?>" />

				<div style="margin-bottom: 15px"></div>

				<label>Date</label>
				<input type="text" name="proj-date" value="<?php echo $_date; ?>" />
	...
	function xt_single_project_options_save() {  
		...
		update_post_meta($post->ID, "proj-client", $_POST["proj-client"]);
