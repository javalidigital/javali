...
$plugin_url = plugins_url() . "/".XT_SLD_DIR."/admin/tinymce/";

$theme_path = get_template_directory_uri() . '/xt_slider_core/shortcodes/';
?>
...
var tb_dialog_helper = {
    ...
    setUpButtons: function () {
        var a = this;
        jQuery("#xt_slider_wp-cancel-button").click(function () {
            a.closeDialog()
        });
        jQuery("#xt_slider_wp-insert-button").click(function () {
            a.insertAction()
        });
    },
    loadShortcodeDetails: function () {
        if (xt_slider_wpSelectedShortcodeType) {
            var a = this;
            jQuery.getScript(shortcode_generator_url + "shortcodes/" + xt_slider_wpSelectedShortcodeType + ".js", function () {
                a.initializeDialog();
                ...
                switch ( xt_slider_wpSelectedShortcodeType ) {
					...
					case 'ordered_list':
					case 'typography':
						jQuery('input#xt_slider_wp-value-content').val( selectedText );
					case 'toggle':
						jQuery('textarea#xt_slider_wp-value-content').val( selectedText );
					break;
                ...
                jQuery.getScript(shortcode_generator_url_theme + "shortcodes/" + xt_slider_wpSelectedShortcodeType + ".js", function () {
                    a.initializeDialog();
    ...
    initializeDialog: function () {
        ...
        if (typeof xt_slider_wpShortcodeAtts == "undefined") {
            jQuery("#xt_slider_wp-shortcode-options").append("<p>Error loading details for shortcode: " + xt_slider_wpSelectedShortcodeType + "</p>");
        } else {
            ...
            var a = xt_slider_wpShortcodeAtts.attributes,
                b = jQuery("#xt_slider_wp-options-table");

            for (var c in a) {
                var f = "xt_slider_wp-value-" + a[c].id,
                    d = a[c].isRequired ? "xt_slider_wp-required" : "",
                ...
                switch (d) {
					...
					*/
                    ...
                    case "sliders-control":
                        this.createSlidersControl(a[c], f, c == 0);
                        break;
	            ...
	            jQuery(".xt_slider_wp-focus-here:first").focus()
	
				// Add additional wrappers, etc, to each select box.
				jQuery('#xt_slider_wp-shortcode-options select').wrap( '<div class="select_wrapper"></div>' ).before('<span></span>');
				jQuery('#xt_slider_wp-shortcode-options select option:selected').each( function () {
    ...
    createTabControl: function (a, b, c) {
        new xt_slider_wpTabMaker(b, 6, c ? "xt_slider_wp-focus-here" : null);
        b.addClass("xt_slider_wp-marker-tab-control")
    },
    ...
    createTextControl: function (a, b, c) {
        ...
        var f = a.validateLink ? "xt_slider_wp-validation-marker" : "",
            d = a.isRequired ? "xt_slider_wp-required" : "",
            g = "xt_slider_wp-" + a.id,
			tipid = "tip-" + a.id,
            e = a.value; // Default text input value (Added 10/10/2011)

        jQuery('<input type="text">').attr("id", g).attr("name", g).val(e).addClass(f).addClass(d).addClass('txt input-text').addClass(c ? "xt_slider_wp-focus-here" : "").appendTo(b);
        ...
        if (a = a.help) {
            if(a != '') {
                ...
                jQuery("<span/>").addClass("xt_slider_wp-input-help").html('<a href="#" id="'+idLink+'" class="tooltip-link"><img src="<?php echo $plugin_url; ?>/help.png" alt="Help" /></a><span class="tooltip" id="'+idText+'"><span class="tool-text">'+a+'</span></span>').appendTo(b);
    ...
    createTextAreaControl: function (a, b, c) {
        ...
        jQuery('<textarea>').attr("id", g).attr("name", g).attr("rows", 10).attr("cols", 30).addClass(f).addClass(d).addClass('txt input-textarea').addClass(c ? "xt_slider_wp-focus-here" : "").appendTo(b);
        b.addClass("xt_slider_wp-marker-textarea-control");
    ...
    createIconControl: function (a, b, c) {
        ...
        var selectNode = jQuery('<select>').attr("id", g).attr("name", g).addClass(f).addClass(d).addClass('select input-select').addClass(c ? "xt_slider_wp-focus-here" : "");

        b.addClass('xt_slider_wp-marker-select-control');
        ...
        /*** ADD ICON SELECTOR ***/
            ...
            for (v in selectBoxValues) {
                ...
                /*
                b.find('#icon-selector-xt_slider_wp-icon').append('<p>'+value+'</p>');
                */
    ...
    createSlidersControl: function (a, b, c) {
        ...
        <?php
            ...
            $query_sliders = new WP_Query( array('post_type' => 'xt_sliders') );

            if($query_sliders->have_posts()) { 
                while($query_sliders->have_posts()) : $query_sliders->the_post();
                 global $post; 
                 $_slug = $post->post_name;
                ?>
                    selectNode.append('<option value="<?php echo $_slug; ?>"><?php the_title(); ?></option>');
                <?php
                endwhile;
            }
        ?>
    ...
    getColumnKeyValue: function (a) {
        var b = a.find("#xt_slider_wp-column-text").text();
    ...
    getTabKeyValue: function (a) {
        var b = a.find("#xt_slider_wp-tab-text").text();
        if (a = Number(a.find("select option:selected").val())) return {
            key: "data",
            value: {
                content: b,
    ...
    makeShortcode: function () {
        ...
        jQuery("#xt_slider_wp-options-table td").each(function () {

            var h = jQuery(this),
                e = null;

            if (e = h.hasClass("xt_slider_wp-marker-select-control") ? b.getSelectKeyValue(h) : b.getTextKeyValue(h)) a[e.key] = e.value
            if (e = h.hasClass("xt_slider_wp-marker-tab-control") ? b.getTabKeyValue(h) : b.getTextKeyValue(h)) a[e.key] = e.value
            if (e = h.hasClass("xt_slider_wp-marker-textarea-control") ? b.getTextAreaKeyValue(h) : b.getTextKeyValue(h)) a[e.key] = e.value

        });

        if (xt_slider_wpShortcodeAtts.customMakeShortcode) return xt_slider_wpShortcodeAtts.customMakeShortcode(a);
        var c = a.content ? a.content : xt_slider_wpShortcodeAtts.defaultContent,
            f = "";
        for (var d in a) {
            var g = a[d];
        ...
        switch ( xt_slider_wpShortcodeAtts.shortcodeType ) {
        
        	case 'text-replace':
        	
        		var shortcode = "[xt_" + xt_slider_wpShortcodeAtts.shortcode + f + "]" + (c ? c + "[/xt_" + xt_slider_wpShortcodeAtts.shortcode + "]" : " ")
    ...
    previewAction: function (a) {
    	...
    	jQuery('#xt_slider_wp-options-table').find('select.input-select-font').each ( function () {
    	
    		fontValue = jQuery(this).val();
    	
    	});
    
        jQuery(a).hasClass("xt_slider_wp-validation-marker") && this.validateLinkFor(a);
    },

    validateLinkFor: function (a) {
        var b = jQuery(a);
        b.removeClass("xt_slider_wp-validation-error");
        b.removeClass("xt_slider_wp-validated");
        if (a = b.val()) {
            b.addClass("xt_slider_wp-validating");
            jQuery.ajax({
                url: ajaxurl,
                dataType: "json",
                data: {
                    action: "xt_slider_wp_check_url_action",
                    url: a
                },
                error: function () {
                    b.removeClass("xt_slider_wp-validating")
                },
                success: function (c) {
                    b.removeClass("xt_slider_wp-validating");
                    c.error || b.addClass(c.exists ? "xt_slider_wp-validated" : "xt_slider_wp-validation-error")
                }
            })
        }
    }
